...
export default function App() {
  var [activeTab, setActiveTab] = useState<TabId>('summary')
  var data = useBuildingData()

  return (
    <div className="min-h-screen bg-[#f5f5f0]">
      {/* Header */}
      <header className="bg-[#1B4332] shadow-lg">
        <div className="px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button className="text-white/70 hover:text-white"><Menu className="h-5 w-5" /></button>
            <div className="flex items-center gap-2.5">
              <div className="flex h-8 w-8 items-center justify-center rounded bg-[#2D7A4F]">
                <Building2 className="h-4 w-4 text-white" />
              </div>
              <span className="font-display text-lg text-white tracking-wide">daisy</span>
          ...
          <div className="relative">
            <div className="flex items-center gap-2 rounded-lg bg-[#244A3A] px-4 py-2 border border-[#2D5A45]">
          ...
          <div className="flex items-center gap-5">
            <button className="flex items-center gap-1.5 text-white/70 hover:text-white text-xs"><Share2 className="h-3.5 w-3.5" />Publish Links</button>
            <button className="flex items-center gap-1.5 text-white/70 hover:text-white text-xs"><MessageSquare className="h-3.5 w-3.5" />Comment</button>
            <button className="flex items-center gap-1.5 text-white/70 hover:text-white text-xs"><Settings2 className="h-3.5 w-3.5" />Actions</button>
          </div>
        </div>
      </header>
      ...
      <footer className="border-t border-gray-200 bg-white px-5 py-3 mt-8">
        <div className="flex items-center justify-between text-xs text-gray-400">
          <span>Daisy Corp Finance · Budget Scenario Planner</span>
          <span>FY {data.selectedBuilding?.budget_year || '2026'}</span>
        </div>
      </footer>
    </div>
  )
